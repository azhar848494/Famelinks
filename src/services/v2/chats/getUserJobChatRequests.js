const {
  getUserJobChatRequests,
  getUserJobChatRequestsCount,
} = require("../../../data-access/v2/chats");
const { getOneUser } = require("../../../data-access/v2/users");

module.exports = async (userId, page, blockList) => {
  const chats = await getUserJobChatRequests(userId, page);
  const result = await getUserJobChatRequestsCount(userId);
  const chatsCount =
    result[0] && result[0].requestCount ? result[0].requestCount : 0;
